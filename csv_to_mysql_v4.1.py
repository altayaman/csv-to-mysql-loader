...
# ---------------------------------------------------------------------------------------------------------
SCHEMA_NAME = 'research_ops_sandbox'
TABLE_NAME  = ''
...
class CSV_FILE:
    ...
    def check_file_path(self, csv_file_path):
        if(csv_file_path == ''):
            ...
            print(' Need to pass csv file name to be imported into MySQL')
            sys.exit(0)
        else:
    ...
    def start_connection(self):
        self.url    = ''.join(['mysql+mysqlconnector://', username_, ":", password_, "@", host_, ':',port_, '/', database_])
    ...
    def get_mysql_types(self, column_types):
        ...
        for col, type in column_types.items():
            if(type == 'Text'):
                column_types_mysql[col] = 'varchar(500)'
            elif(type == 'Integer'):
                column_types_mysql[col] = 'INT'
            elif(type == 'BigInteger'):
                column_types_mysql[col] = 'BIGINT'
...
class Csv_into_Mysql:
    ...
    def legitimate_as_table_name(self, table_name_str_):
        table_name_alphanumeric_str_ = replace_chars_by(table_name_str_, replace_char_by_='_', keep_='alphanum')
